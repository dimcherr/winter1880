...
void work::DrawHackerKeyboard() {
    ...
    hub::Reg().view<TextWidget, BoundsWidget, HackerKey, tag::Hacker>().each([](TextWidget& text, BoundsWidget& bounds, HackerKey& hackerKey) {
        if (!bounds.visible) return;
        auto& keys = State::Get().keys;

        if (hackerKey.timeSincePressed > hackerKey.period) {
            hackerKey.timeSincePressed = hackerKey.period;
        } else {
            hackerKey.timeSincePressed += hub::GetDeltaTime();
        }
        

        if (keys[(int)hackerKey.key] && hackerKey.timeSincePressed >= hackerKey.period) {
            hackerKey.timeSincePressed = 0.f;
            if (hackerKey.onPress) {
                hackerKey.onPress(hackerKey.key);
            }
        }


        text.text = hackerKey.symbol;
        ...
        float animAlpha = hackerKey.timeSincePressed / hackerKey.period;
        ...
        gl::State().drawMaterial.fsParams.color = tun::Lerp(Vec4(tun::green, 1.f), bounds.color, animAlpha);
        gl::UseMesh(rectMesh.vertexBuffer, rectMesh.indexBuffer, rectMesh.elementCount);
        gl::UpdateDrawMaterial();
        gl::Draw();


        float deformAmp = tun::Lerp(0.001f, 0.00005f, tun::Clamp(animAlpha * hackerKey.animIntensity, 0.f, 1.f));
        float scaleAmp = tun::Lerp(3.f, 0.1f, tun::Clamp(animAlpha * hackerKey.animIntensity, 0.f, 1.f));
        float deformTime = tun::Lerp(1.f, (float)hub::GetTime(), tun::Clamp(animAlpha * hackerKey.animIntensity, 0.f, 1.f));
        ...
        ImGui::PushStyleColor(TundraCol_Offset, {0.f, 0.0f, (panelPos.x + panelSize.x) * 0.5f, (panelPos.y + panelSize.y) * 0.5f});
