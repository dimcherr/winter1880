...
SOKOL_API_IMPL void simgui_setup(const simgui_desc_t* desc) {
    ...
    // TODO TUNDRA INTERVENTION!!!
    //shd_desc.vertex_func.source = _simgui_vs_source_dummy;
    //shd_desc.fragment_func.source = _simgui_fs_source_dummy;
    // TODO TUNDRA INTERVENTION

    //_simgui.def_shd = sg_make_shader(&shd_desc);
    //_simgui.def_shd = sg_make_shader(sg_)
    _simgui.def_shd = sg_make_shader(ui_shader_desc(sg_query_backend()));
    ...
    {
        sg_vertex_attr_state* attr = &pip_desc.layout.attrs[3];
        attr->offset = offsetof(ImDrawVert, offset);
        attr->format = SG_VERTEXFORMAT_FLOAT4;
    }
    {
        sg_vertex_attr_state* attr = &pip_desc.layout.attrs[4];
        attr->offset = offsetof(ImDrawVert, deform);
        attr->format = SG_VERTEXFORMAT_FLOAT4;
    }
